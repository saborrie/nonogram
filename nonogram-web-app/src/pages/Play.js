...
function Play(props) {
  ...
  const [solution, setSolution] = React.useState(
    List(Array(width * height).fill(false))
  );
  ...
  const columnClues = range(width).map(columnIndex => {
    const columnValueIndices = range(height).map(
      rowIndex => rowIndex * width + columnIndex
    );

    const clue = createClue(columnValueIndices.map(i => solution.get(i)));
    const currentValuesClue = createClue(
      columnValueIndices.map(i => currentValues[i] === 1)
    );
  ...
  const rowClues = range(height).map(rowIndex => {
    const rowValueIndices = range(width).map(
      columnIndex => rowIndex * width + columnIndex
    );

    const clue = createClue(rowValueIndices.map(i => solution.get(i)));
    const currentValuesClue = createClue(
      rowValueIndices.map(i => currentValues[i] === 1)
    );
    const complete = clue.join(";") === currentValuesClue.join(";");
  ...
  return (
    <React.Fragment>
      <Announcement>
        ...
        <AnnouncementAction>
          <Button onClick={() => dispatchMoveAction({ type: "clear" })}>
            Play again
          </Button>
        </AnnouncementAction>
        <AnnouncementAction>
        ...
        <GameBoardRow>
          <PreviewBox>
            <PreviewSvg
              width={width}
              height={height}
              pattern={currentValues.map(v => v === 1)}
            />
          </PreviewBox>
          {range(width).map(columnIndex => (
            <Clue
              key={columnIndex}
              variant="column"
              completed={columnClues[columnIndex].complete}
            >
      ...
      <ControlsBox>
        ...
        <ControlsBoxItem>
          <Button onClick={() => dispatchMoveAction({ type: "undo" })}>
            Undo
          </Button>
        </ControlsBoxItem>
        <ControlsBoxItem>
